<?php
 ...
 $sql = "SELECT ID,RoleName,GradingSelection,Status,DATE_FORMAT(DateAdded, '%M %d %Y') as Added,DATE_FORMAT(DateModified, '%M %d %Y') as Modified FROM roles WHERE ID <> 0";
 $res = mysqli_query($con,$sql) or die(mysqli_error($con));
 
 if(isset($_POST['fordelete']) && $_POST['fordelete']=='true')
 {
	 $GradingSelection=array();
    if(isset($_POST['GradingSelection']) && is_array($_POST['GradingSelection']))
	{ 
	...
	// echo $keyvalues;
		$sql = "DELETE FROM roles WHERE ID IN (".$keyvalues.")";
		// echo $sql;
