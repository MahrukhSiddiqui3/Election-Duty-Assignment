...
if(isset($_POST['issubmit']) && $_POST['issubmit'] == 'true')
{
   ...
   if(isset($_POST['MaleBooth']))
	...
	if($UserID == 0)
		...
		Please Select Officer Name
		</div>';
	if($StationName == '')
		$msg = '<div class="alert alert-danger alert-dismissible">
		<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
		Please Enter StationName Of Polling Station
		</div>';
	elseif($Address == '')
		$msg = '<div class="alert alert-danger alert-dismissible">
		<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
...
else
{
 ...
 $sql = "SELECT ID,UserID,StationName,Address,MaleBooth,FemaleBooth,Status,DATE_FORMAT(DateAdded, '%M %d %Y') as Added,DATE_FORMAT(DateModified, '%M %d %Y') as Modified FROM pollingstations WHERE ID = '".$EID."'";
 // echo $sql;
 ...
 $MaleBooth = $rows['MaleBooth'];
 $FemaleBooth = $rows['FemaleBooth'];
}
            ...
            <div class="form-group">
			 ...
			 <div class="form-group">
                  <label for="exampleInputEmail1"> User</label>
					...
					{
					?>
						<option <?php echo ($UserID == $rows['ID'] ? 'selected' : '');?> value="<?php echo $rows['ID']?>"><?php echo $rows['Name']?></option>
					<?php
					}
					?>
                  </select>
