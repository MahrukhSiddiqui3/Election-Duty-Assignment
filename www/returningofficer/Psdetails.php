<?php
 ...
 $sql="SELECT p.ID,p.StationName,p.Address,p.NumberOfBooths,p.UserID,p.Status,DATE_FORMAT(p.DateAdded, '%M %d %Y') as Added,DATE_FORMAT(p.DateModified, '%M %d %Y') as Modified FROM pollingstations p LEFT JOIN users u ON p.UserID = u.ID where p.ID <> 0 AND p.UserID=".$_SESSION['UserID']." AND p.ID = ". $EID."";
...
?>
  ...
  <div class="content-wrapper">
    ...
    for($i=1 ;$i<=$rows['NumberOfBooths']; $i++)
